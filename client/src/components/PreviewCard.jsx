...
const PreviewCard = ({ shoe }) => {
  ...
  let parsedSizeRange;
      ...
      const cleanedSizeRangeString = sizeRangeString.replace(/^{(.*)}$/, "$1");
      const jsonString = `[${cleanedSizeRangeString}]`;
      parsedSizeRange = JSON.parse(jsonString);

      // Làm tròn và bỏ số trùng và sắp xếp
      const sizes = parsedSizeRange
          .map(parseFloat)
          .map(Math.round)
